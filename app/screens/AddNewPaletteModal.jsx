...
const ColorItem = ({ item, onSelect }) => {
  ...
  const handleSwitch = () => {
    ...
    onSelect((prev) => {
      if (isSelected) {
        return prev.filter((color) => color.colorName !== item.colorName);
      } else {
        return [...prev, item];
      }
    });
  ...
  return (
    <View style={styles.listItem}>
      ...
      <Switch
        value={isSelected}
        trackColor={{ false: "#767577", true: "blueviolet" }}
        thumbColor={isSelected ? "indigo" : "#f4f3f4"}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        data={COLORS}
        renderItem={({ item }) => (
          <ColorItem item={item} onSelect={setSelectedColors} />
        )}
